...
function createArtistsCartTemplate(artist) {
  ...
  return `
    <li class="artist-card" data-artist-id="${artist._id}">
      <img class="artist-photo" src="${artist.strArtistThumb || ''}" alt="${artist.strArtist || 'Artist photo'}" loading="lazy">
      <ul class="artist-genre-list">
        ${genreList}
      </ul>
      <h4 class="artist-name">${artist.strArtist || 'Unknown Artist'}</h4>
      <p class="artist-info">${artist.strBiographyEN ? artist.strBiographyEN.slice(0, 100) + '...' : 'No biography available'}</p>
...
export function createArtistInfo(artist) {
    ...
    : artist.albumsList.map(album => {
        const trackList = !album.tracks || album.tracks.length === 0
          ...
          : album.tracks.map(track => `
            <li class="track">
              <span class="track-name">${track.strTrack || 'Unknown Track'}</span>
              <span class="track-time">${msToMinSec(track.intDuration)}</span>
              <a class="track-link" href="${!track.movie || track.movie === "null" ? '#' : track.movie}" target="_blank">
  ...
  const markup = `
    ...
    <div class="modal-artist-about">
      <img class="modal-artist-photo" src="${artist.strArtistThumb || ''}" alt="${artist.strArtist || 'Artist'}" loading="lazy">
      <div>
        ...
        <ul class="modal-artist-list">${genreList}</ul>
      </div>
    </div>
    <h3 class="modal-artist-album-title">Albums</h3>
    <ul class="modal-artist-album">${albumList}</ul>
  `;
